...
- **L-8. The release-based reconciliation policies need a lease to be concurrency-safe.** `check_then_decide` and `auto_retry` are **crash-recovery** tools: they assume the worker that left a key `IN_FLIGHT` is *gone*. An ownership/fencing token on every claim makes `release` a compare-and-delete, so a stale reconciler can never delete a newer claim. Without a lease they remain single-reconciler: if *multiple live workers* reconcile the same key at once, a "not committed *yet*" verdict cannot distinguish a slow live worker from a dead one, and more than one may run — so use `quarantine` (default) or `wait`, which are always concurrency-safe. **With a lease** (`once(..., lease_ttl=...)`, v0.2) the gap closes: the owner renews a lease on the store's clock via a background heartbeat, a reconciler adopts an orphan only once its lease has expired, and the ownership token makes the takeover single-winner. A live owner is never adopted; a dead one's orphan is, by exactly one reconciler.
